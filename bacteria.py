...
class bacteria():
    ...
    def tumboNadoDinamico(self, numGaps, poblacion):
        bestFitness = max(poblacion, key=lambda x: x.fitness).fitness
        fitnessDiff = bestFitness - self.fitness

        # Ajuste del número de gaps basado en la diferencia de fitness
        if fitnessDiff > 50:  # Si la diferencia es muy grande, ajustar más agresivamente
            numGapsAdaptado= numGaps + 3
        elif fitnessDiff > 20:  # Ajuste moderado
            numGapsAdaptado = numGaps + 1
        else:  # Si la diferencia es pequeña, mantener el número de gaps inicial
            numGapsAdaptado = numGaps

        numGapsAdaptado = max(1, numGapsAdaptado)  # Asegurarse de que al menos se inserte un gap
        self.tumboNado(numGapsAdaptado)
